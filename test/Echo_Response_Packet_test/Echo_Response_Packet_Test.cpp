...
void test_compile_response_to_correct_packet(void)
{
    uint16_t random_test_value = 13;
    CAN_Frame test_frame = Echo_Response_Packet(random_test_value);
...
void test_parse_response_to_correct_packet(void)
{
    uint16_t random_test_value = 14;
    Echo_Response_Packet packet(random_test_value);
...
{
    uint16_t random_test_value = 14;
    CAN_Mock::set_packet_rx(Echo_Response_Packet(random_test_value));
    ...
    latest_response_packet.parse_to_core_values(&received_value);
...
int main(int argc, char **argv)
{
    ...
    RUN_TEST(test_compile_response_to_correct_packet);
    RUN_TEST(test_parse_response_to_correct_packet);
